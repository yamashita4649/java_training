...
public class TypeDese {
    public static void main(String[] args) {
        TypeDese dese = new TypeDese();
        for(String name : args) {
            Class<?> startClass = null;
            try {
                startClass = Class.forName(name);
                dese.printType(startClass,0,basic);
    ...
    basic = {"class","interface","enum","annotation","nest"},
    ...
    private void printType(Type type, int depth, String[] labels) {
        ...
        if(nestClass != null) {
            out.print(labels[4] + " ");
            out.print(nestClass.getCanonicalName());
        }
